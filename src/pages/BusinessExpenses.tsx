...
export default function BusinessExpenses() {
  ...
  return <div className="space-y-6 pb-8">
    ...
    <div className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]"><form onSubmit={submit} className="app-surface space-y-4 p-5 pb-24 sm:p-6 sm:pb-6"><h3 className="section-title">Add expense</h3><div><label className="form-label">Date</label><input type="date" className="form-input" value={date} onChange={(e) => setDate(e.target.value)} /></div><div><label className="form-label">Expense Type</label><select className="form-input" value={category} onChange={(e) => setCategory(e.target.value)}>{categories.map((item) => <option key={item}>{item}</option>)}</select></div><div><label className="form-label">Amount</label><input type="number" min="0" step="0.01" className="form-input" value={amount || ''} onChange={(e) => setAmount(Number(e.target.value || 0))} placeholder="₹0" /></div><div><label className="form-label">Note (optional)</label><textarea className="form-input min-h-[90px]" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Details about this expense" /></div><div className="fixed inset-x-0 bottom-0 z-40 border-t border-[#eadff2] bg-white/95 p-3 shadow-[0_-12px_30px_rgba(52,31,63,0.12)] backdrop-blur sm:static sm:border-0 sm:bg-transparent sm:p-0 sm:shadow-none"><button className="primary-btn w-full" disabled={saving || amount <= 0}>{saving ? 'SAVING...' : 'SAVE EXPENSE'}</button></div></form>
